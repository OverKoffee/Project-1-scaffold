...
ADD:
 Will have a text field to insert the word to add to their review deck:
    - after setting the word to be learned (added to the deck),
    - a default front card appears with a default "example sentence"
        - this can be replaced if the user inputs their own contextual sentence
    - a default back card appears with a default definition of the word

REVIEW:
 Front card text appears with the sentence containing word to be learned
    - the user will decide if they understand it or don't
    - if user chooses "Good", the card is removed from review stack
    - if user chooses "Bad", the card is returns to the review stack
